...
async function checkDb() {
  ...
  try {
    ...
    console.log(`Tenants found: ${tenants.rowCount ?? tenants.rows.length}`);

    // Check users
    const users = await client.query('SELECT id, email, name, role, tenant_id FROM users');
    console.log(`Users found: ${users.rowCount ?? users.rows.length}`);

    // Check events
    const events = await client.query('SELECT id, name, tenant_id, organizer_id FROM events');
    console.log(`Events found: ${events.rowCount ?? events.rows.length}`);
