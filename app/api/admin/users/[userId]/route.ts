// ============================================
// Galeria - Supervisor User Detail API
// ============================================
...
export async function GET(
  ...
  try {
    ...
    return createAdminDataResponse(user);
  } catch (error) {
    console.error('[ADMIN_USER_DETAIL] Error:', error);
    return createAdminErrorResponse('Failed to load user', 'INTERNAL_ERROR');
  }
}
...
export async function PATCH(
  ...
  try {
    ...
    if (nextSubscriptionTier !== undefined) {
      await logSimpleAdminAction(request, auth.user, 'user.tier_changed', {
